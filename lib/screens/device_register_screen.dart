...
class _DeviceRegisterScreenState extends State<DeviceRegisterScreen> {
  ...
  Future<void> _scanWifiNetworks() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(
        () => _wifiScanError = e.toString().replaceFirst('Bad state: ', ''),
      );
    } finally {
  ...
  Future<void> _registerDevice() async {
    ...
    if (serial.isEmpty || deviceId.isEmpty) {
      AppDialogs.showErrorSnackBar(
        context,
        'Serial number and device ID are required.',
      );
      return;
    }
    ...
    if (!RegExp(r'^DISP_[0-9]{4,}$').hasMatch(deviceId)) {
      AppDialogs.showErrorSnackBar(
        context,
        'Device ID must be in the format DISP_0001',
      );
      return;
    }
    ...
    if (token == null || token.isEmpty) {
      AppDialogs.showErrorSnackBar(
        context,
        'Please login first to register a device.',
      );
      return;
    }
    ...
    try {
      final response = await ServiceLocator.instance.deviceService
          .registerDevice(
            serialNumber: serial,
            deviceId: deviceId,
      ...
      final registeredDeviceId = response['device']?['id']?.toString();
      _deviceId = registeredDeviceId;
  ...
  Future<void> _provisionToDevice() async {
    if (_deviceToken == null || _deviceId == null) {
      AppDialogs.showErrorSnackBar(
        context,
        'Register device first to get device token.',
      );
      return;
    }
    ...
    if (ssid.isEmpty || password.isEmpty) {
      AppDialogs.showErrorSnackBar(
        context,
        'Wi‑Fi SSID and password are required to provision.',
      );
      return;
    }
    ...
    try {
      ...
      if (testResp == null) {
        ...
        AppDialogs.showErrorSnackBar(
          context,
          'Cannot reach ESP32. Check connection steps above.',
        );
        return;
      }
      ...
      if (resp.statusCode >= 200 && resp.statusCode < 300) {
        AppDialogs.showSuccessSnackBar(
          context,
          'Provisioning data sent to ESP32.',
        );
        setState(() {
          _message =
              '✅ Success! Device will connect to home WiFi now.\n'
              'Check backend in 30-60 seconds for device online status.';
        });
      } else {
        AppDialogs.showErrorSnackBar(
          context,
          'Provision failed: ${resp.statusCode}',
        );
        setState(() {
          _message =
              '❌ ESP32 rejected request (Status: ${resp.statusCode})\n'
              'Response: ${resp.body}';
        });
      }
      ...
      setState(() {
        _message =
            '⏱️ Timeout: ESP32 not responding\n\n'
            '✓ Troubleshooting:\n'
            '1. Restart ESP32\n'
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (!_isWifiScanning &&
                          ...
                          Text(
                            'No networks detected. Turn on Wi-Fi and scan again.',
                            style: AppTextStyles.bodySmall.copyWith(
                              color: AppColors.textSecondary,
                            ),
                          ),
