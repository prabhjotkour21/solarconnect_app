...
class _SavingsScreenState extends State<SavingsScreen> {
  ...
  Future<void> _loadSavingsData() async {
    ...
    try {
      ...
      final summary =
          (response['summary'] ?? <String, dynamic>{}) as Map<String, dynamic>;
      final investmentResponse = await ServiceLocator
          .instance
          .userProfileService
          .getProfile(token);
      final userInvestment =
          (investmentResponse['investmentAmount'] ??
                  investmentResponse['data']?['investmentAmount'] ??
                  0)
              .toDouble();
  ...
  List<Map<String, dynamic>> get _graphDataList =>
      _graphBars.isNotEmpty ? _graphBars : const [];
  ...
  Future<void> _showInvestmentDialog() async {
    ...
    if (token == null || token.isEmpty) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please login again to update investment.'),
          ),
        );
      }
      return;
    ...
    try {
      ...
      await ServiceLocator.instance.savingsService.updateInvestmentAmount(
        token,
        val,
      );
  ...
  Future<void> _pickGraphCustomRange() async {
    final picked = await showDateRangePicker(
      ...
      builder: (ctx, child) => Theme(
        data: Theme.of(
          ctx,
        ).copyWith(colorScheme: ColorScheme.dark(primary: AppColors.success)),
        child: child!,
      ),
    );
    if (picked != null) {
